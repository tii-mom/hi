...
export default function LandingPage() {
  ...
  const copy = (item: LandingCopy) => t(item.key, item.fallback);
  ...
  return (
    <div className="min-h-screen bg-bg-base text-text-primary overflow-hidden selection:bg-accent-blue/30 relative">
      ...
      <div
        className="absolute bottom-1/4 -right-1/4 w-[600px] h-[600px] bg-accent-violet/10 rounded-full blur-[120px] mix-blend-screen pointer-events-none animate-breathe"
      ...
      <main className="relative z-10 pt-48 pb-24 px-6 max-w-[1400px] mx-auto flex flex-col items-center text-center">
        ...
        >
          ...
          <span className="text-[10px] font-mono tracking-[0.2em] uppercase text-text-secondary">
            {t('landing.sync')}
          </span>
        </motion.div>
        ...
        >
          {t('landing.title1')} <br />
          <span className="text-text-secondary font-serif italic text-5xl md:text-7xl lg:text-[80px]">
            {t('landing.title2')}
          </span>{' '}
          <span className="relative">
            <span className="absolute inset-0 blur-[40px] bg-accent-blue/30 text-transparent">
              {t('landing.title3')}
            </span>
            {t('landing.title3')}
          </span>
        ...
        >
          ...
          <a
            href="#agents"
            className="h-16 px-10 rounded border border-white/10 bg-transparent hover:bg-white/5 text-white font-mono text-xs uppercase tracking-widest transition-colors flex items-center justify-center"
          >
            {t('landing.observe')}
          </a>
        </motion.div>
        ...
        >
          ...
          <svg className="absolute inset-0 w-full h-full opacity-30 animate-breathe" viewBox="0 0 1000 500">
            <path
              d="M500 250 L200 150 M500 250 L800 150 M500 250 L200 400 M500 250 L800 400"
              stroke="white"
              strokeWidth="1"
              strokeDasharray="4 4"
              fill="none"
            ...
            <circle
              ...
              strokeWidth="1"
              fill="none"
              className="opacity-20 animate-pulse-ring-blue"
            />
            <circle
              cx="500"
              cy="250"
              ...
              strokeWidth="1"
              fill="none"
              className="opacity-10 animate-pulse-ring-blue"
          ...
          {/* Surrounding Nodes */}
          {landingViewModel.heroSecondaryNodes.map((node) => {
            const Icon = heroNodeIcons[node.icon];
            ...
            return (
              ...
              >
                ...
                <div className="mt-3 px-3 py-1 bg-black/40 border border-white/5 rounded backdrop-blur-md whitespace-nowrap">
                  <span className="text-[9px] uppercase tracking-widest font-mono text-text-secondary">
                    {copy(node.label)}
                  </span>
                </div>
              </div>
          ...
          <div className="absolute bottom-6 left-6 right-6 flex justify-between items-end pointer-events-none">
            <div className="bg-black/50 border border-white/10 p-4 rounded-xl backdrop-blur-md w-64">
              ...
              <div className="text-white text-xs leading-relaxed font-light">"{t('ui.executing')}"</div>
            </div>
      ...
      <section className="relative z-10 py-32 px-6 border-t border-border bg-black/50">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-12">
          {landingViewModel.valueProps.map((valueProp) => {
            const Icon = valuePropIcons[valueProp.icon];
            const toneClasses = valuePropToneClasses[valueProp.tone];

            return (
              <div key={valueProp.id} className="space-y-4">
                <div
                  className={`w-12 h-12 rounded-xl ${toneClasses.wrapper} flex items-center justify-center border ${toneClasses.border}`}
                >
                  <Icon className={`w-6 h-6 ${toneClasses.icon}`} />
                </div>
                <h3 className="text-xl font-medium">{copy(valueProp.title)}</h3>
                <p className="text-text-secondary leading-relaxed">{copy(valueProp.description)}</p>
              </div>
            );
          })}
        </div>
